...
import com.bigos.payment.application.payment.PaymentApplicationService;
import com.bigos.payment.application.payment.exception.PaymentApplicationException;
...
public class OrderCancellingEventKafkaListener implements KafkaConsumer<OrderCancellingEventDtoKafka> {
    ...
    public OrderCancellingEventKafkaListener(final PaymentApplicationService paymentApplicationService,
                                             final InputMessagingKafkaDataMapper inputMessagingKafkaDataMapper) {
        ...
        messages.forEach(orderCancellingEvent -> {
            ...
            try {
                if (CANCELLING.name().equals(orderCancellingEvent.getData().status())) {
                    paymentApplicationService.cancelPayment(inputMessagingKafkaDataMapper
